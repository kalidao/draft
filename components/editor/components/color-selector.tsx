      ...
      <div className="relative h-full">
        ...
        >
          <span
            ...
            style={{
              color: activeColorItem?.color ?? 'white',
              backgroundColor: activeHighlightItem?.color ?? 'black',
            }}
          >
            A
          </span>
          ...
          {TEXT_COLORS.map(({ name, color }, index) => (
            <button
              ...
              onClick={() => {
                ...
                name !== "Default" &&
                  editor.chain().focus().setColor(color ?? 'white').run();
                setIsOpen(false);
              }}
          ...
          {HIGHLIGHT_COLORS.map(({ name, color }, index) => (
            <button
              ...
              onClick={() => {
                ...
                name !== "Default" && editor.commands.setHighlight({ color: color ?? 'white' });
                setIsOpen(false);
              }}
              className="flex items-center justify-between rounded-sm px-2 py-1 text-sm text-stone-600 hover:bg-stone-100"
              type="button"
